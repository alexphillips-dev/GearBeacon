...
export async function testUpdateNotice({evaluate,waitForBrowser,assertAccessible,assert,cdp,screenshotRoot}) {
  await evaluate(`(() => {
    ...
    window.noticeFixture={currentVersion:'1.4.0',latestVersion:'1.5.0',channel:'main',verified:true,updateAvailable:false,releaseNotesUrl:'https://github.com/alexphillips-dev/GearBeacon/releases/tag/v1.5.0',releaseNotes:'Example update <notes>',compatibilityWarnings:[]};
  ...
  try {
    ...
    assert(await evaluate("document.getElementById('appUpdateNotice').children.length===1 && document.getElementById('appUpdateButton').textContent==='update available · v1.5.0'"),'Update notice contains extra controls or unexpected text');
    for(const channel of ['main','dev']) {
      await evaluate(`window.noticeFixture.channel='${channel}'; window.noticeFixture.latestVersion='${channel==='dev'?'1.5.0-dev.2':'1.5.0'}'; refresh()`);
      await waitForBrowser(`document.getElementById('appUpdateButton').textContent==='update available · v${channel==='dev'?'1.5.0-dev.2':'1.5.0'}'`,'Notice did not refresh its version');
    ...
    await evaluate("window.noticeFixture.latestVersion='1.5.0-rc.1+build.2'; refresh()");
    assert(await evaluate("!document.getElementById('appUpdateNotice').classList.contains('hidden') && document.getElementById('appUpdateButton').textContent==='update available · v1.5.0-rc.1+build.2'"),'A version with build metadata hid the update button');
    ...
    assert(click?.href.endsWith('/releases/tag/v1.5.0') && click.target==='_blank' && click.rel.includes('noopener') && !click.prevented,'Keyboard activation did not follow the release-notes link safely');
