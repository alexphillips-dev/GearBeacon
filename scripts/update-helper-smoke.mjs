...
if (process.platform === 'win32') {
  run('pwsh', ['-NoProfile', '-NonInteractive', '-File', 'deploy/update-windows.ps1', '-Version', '1.10.0'], 'BackupConfirmed');
} else {
  run('sh', ['deploy/update-mac-linux.sh', '1.10.0'], '--backup-confirmed');
  run('sh', ['deploy/update-docker.sh', '1.10.0'], '--backup-confirmed');
}
