...
if (process.platform === 'win32') {
  run('pwsh', ['-NoProfile', '-NonInteractive', '-File', 'deploy/update-windows.ps1', '-Version', '0.1.10'], 'BackupConfirmed');
} else {
  run('sh', ['deploy/update-mac-linux.sh', '0.1.10'], '--backup-confirmed');
  run('sh', ['deploy/update-docker.sh', '0.1.10'], '--backup-confirmed');
}
