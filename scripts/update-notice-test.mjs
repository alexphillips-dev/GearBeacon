...
const mock=http.createServer(async(req,res)=>{
  ...
  let value;
  if (url.pathname==='/repo/releases/latest') value=scenario.releases || release('1.3.0');
    ...
    value={type:'file',encoding:'base64',content:Buffer.from(JSON.stringify({latestVersion:scenario.version || '1.2.0',releaseNotes:'Development notes',releasePageUrl:release('1.2.0').html_url})).toString('base64')};
...
async function start({channel='main',automatic=false,metadata={},env={},gitBranch=null}={}) {
  ...
  await writeFile(join(appRoot,'build-info.json'),JSON.stringify({version:'1.2.0',packageVersion:'1.2.0',branch:channel,commit:current,...metadata}));
  if (gitBranch) {
...
try {
  ...
  assert.equal(result.channel,'main');assert.equal(result.updateAvailable,true);assert.equal(result.latestVersion,'1.3.0');
  assert.equal(result.releaseNotesUrl,release('1.3.0').html_url);
  ...
  scenario={releases:[release('9.0.0-rc.1'),release('8.0.0',{draft:true}),release('1.2.0')]};
  result=await request();assert.equal(result.latestVersion,'1.2.0');assert.equal(result.updateAvailable,false,'Stable installs received a prerelease or draft');
  scenario={};await request();scenario={failure:503};result=await request();
  assert.equal(result.updateAvailable,true);assert.equal(result.stale,true);assert.equal(result.latestVersion,'1.3.0','Failure erased the last verified update');
  ...
  result=await request();assert.equal(result.channel,'dev');assert.equal(result.updateAvailable,true);assert.equal(result.latestVersion,'1.2.0','Same-version dev commits lost their version label');
  ...
  scenario={version:'1.3.0-dev.2',tagged:true};result=await request();assert.equal(result.releaseNotesUrl,release('1.3.0-dev.2').html_url,'Exact prerelease notes were not preferred');

  scenario={manifest:{channel:'dev',latestVersion:'1.2.0-rc.10',releaseNotesUrl:'javascript:alert(1)'}};
  await start({channel:'dev',metadata:{packageVersion:'1.2.0-rc.2'},env:{GEARBEACON_UPDATE_MANIFEST_URL:source+'/manifest/{channel}'}});
  ...
  assert.equal((await request('/healthz')).packageVersion,'1.2.0-rc.2');assert.deepEqual(calls,['/manifest/dev']);
  scenario={manifest:{channel:'main',latestVersion:'9.0.0'}};result=await request();assert.equal(result.latestVersion,'1.2.0-rc.10','Wrong-channel manifest replaced the verified dev update');assert.equal(result.stale,true);
  ...
  const built=buildMetadata(checkout,'1.2.0',{});assert.equal(built.branch,'dev');assert.match(built.commit,/^[a-f0-9]{40}$/);
  assert.equal(result.currentCommit,built.commit,'Source commit did not override stale package metadata');
  assert.equal(buildMetadata(checkout,'1.2.0-rc.1',{GITHUB_REF:'refs/tags/v1.2.0-rc.1'}).branch,'dev');
  assert.equal(buildMetadata(checkout,'1.2.0',{GITHUB_REF:'refs/tags/v1.2.0'}).branch,'main');
  assert.throws(()=>buildMetadata(checkout,'../unsafe',{}));
  const staged=join(scratch,'source-package');await mkdir(staged);
  execFileSync(process.execPath,[join(root,'scripts','build-metadata.mjs'),staged,'1.2.0'],{cwd:root,stdio:'ignore',windowsHide:true,
  ...
  assert.equal(packaged.branch,'dev');assert.equal(packaged.commit,current);assert.equal(packaged.packageVersion,'1.2.0');
  await start({env:{GEARBEACON_BUILD_BRANCH:'dev',GEARBEACON_BUILD_COMMIT:current,GEARBEACON_PACKAGE_VERSION:'v1.2.0',GEARBEACON_IMAGE:'ghcr.io/alexphillips-dev/gearbeacon:dev'}});
