...
try {
  let status = null;
  for (let i = 0; i < 100; i += 1) { try { const response = await fetch(`http://127.0.0.1:${port}/api/status`); if (response.ok) { status = await response.json(); break; } } catch {} await delay(100); }
  if (status?.version !== '1.6.0' || status?.storage?.schemaVersion !== 5) throw new Error(`Standalone did not start correctly: ${JSON.stringify(status)}`);
