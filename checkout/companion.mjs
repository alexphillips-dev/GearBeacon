...
const terminal = createTerminal(), errorTerminal = createTerminal({ stream:stderr });
...
try {
  ...
  const ask = message => terminal.prompt(prompt=>questions.question(prompt,{ signal:abort.signal }),message);
  if (command === 'profiles') {
    const report = savedProfileReport(state);
    for (const line of report.lines) console.log(line);
    if (!report.ok) process.exitCode = 1;
  } else if (command === 'pair') {
    terminal.section('Pair the checkout companion');
    ...
    terminal.notice('PAIRED',['No address profile has been saved yet.','Next: run npm run connect.'],'success');
  } else {
    ...
    if (command === 'connect' || command === 'verify') {
      terminal.section(command === 'verify' ? 'Verify your saved checkout profile' : 'GearBeacon checkout setup');
      terminal.text('Complete the browser steps, then return here at each Enter prompt.');
      terminal.section('1. Choose a profile');
      ...
      const addressLabel = command === 'verify' ? saved.addressLabel : (await ask('Address nickname (for example, Home): ')).trim();
      ...
      if (saved && command === 'connect') terminal.notice('Replacing an existing profile',[
        'Completing Connect replaces this region’s saved profile.',
        'Review and explicitly reauthorize its purchase rules afterwards.',
      ]);
      terminal.text('Opening the dedicated Store browser...');
      ...
      await setupProfile({ store,region,addressLabel,saved:command === 'verify' ? saved : null,state,vault,ask,terminal,signal:abort.signal });
      if (command === 'connect') {
        try {
          await client('heartbeat',{ protocol:1,mode:state.mode,profiles:publicProfiles(state.profiles) });
          terminal.text('Saved profile reported to GearBeacon.',{ tone:'success' });
        } catch { terminal.notice('Saved locally; dashboard not updated',[
          'Check the dashboard connection. Your saved profile is still on disk.',
          'npm start will report the saved profile again.',
        ]); }
      }
      await store.close();
      terminal.notice('Next step',command === 'verify'
        ? ['Verification did not refresh the saved login or rearm any purchase.','Use Connect if the saved session needs updating.']
        : ['Run npm start and leave this companion running.','Then set up auto-buy on a watched product.'],'heading');
    } else {
  ...
  if (abort.signal.aborted) errorTerminal.notice(command === 'connect' ? 'Setup cancelled' : command === 'verify' ? 'Verification cancelled' : 'Checkout companion stopped',command === 'connect'
    ? ['A new profile is saved only after the SAVED confirmation.','Run npm run profiles to check what is on disk.']
    : command === 'verify' ? ['The saved profile is unchanged.','Remove any setup item before restarting purchasing.'] : []);
  else errorTerminal.notice('Checkout companion stopped',err instanceof CommandError ? [err.message]
    ...
    : ['Verify installation, private vault access, pairing, and browser availability.','Run npm run profiles to check saved setup.','Check uncertain orders in Settings > General > Auto-buy before restarting.']);
  process.exitCode = 1;
