...
if (/[\r\n]/.test(EMAIL_SUBJECT_PREFIX)) throw new Error('GEARBEACON_EMAIL_SUBJECT_PREFIX must not contain line breaks.');
try { new Intl.DateTimeFormat('en-US', { timeZone:NOTIFICATION_TIME_ZONE }).format(); }
catch { throw new Error('GEARBEACON_TIME_ZONE must be a valid IANA timezone, such as America/New_York.'); }
...
function normalizeWatchRule(input, base = DEFAULT_WATCH_RULE) {
  ...
  if (value.targetPrice === null || value.targetPrice === '') normalized.targetPrice = null;
    ...
    if (!Number.isFinite(target) || target < 0 || target > 1000000) throw new Error('Target price must be a positive number.');
    normalized.targetPrice = Math.round(target * 100) / 100;
  }
  if (value.pausedUntil === null || value.pausedUntil === '') normalized.pausedUntil = null;
  else if (value.pausedUntil === 'indefinite') normalized.pausedUntil = 'indefinite';
...
function watchRule(slug, region = currentRegion()) {
  if (!tableExists('watch_rules')) return { ...DEFAULT_WATCH_RULE };
...
function recordProductObservation(product, changeType = 'observed', region = currentRegion()) {
  if (!product || !tableExists('product_observations')) return;
  ...
  if (previous && previous.status === product.status && Boolean(previous.in_stock) === Boolean(product.inStock) && (previous.price_text || null) === (product.price || null)) return;
...
function secondaryBackupOwnerId() {
  ...
  if (saved && /^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(saved)) return saved;
...
function secondaryBackupNames() {
  const owned = new Set();
  if (!tableExists('backup_log')) return owned;
  for (const row of db.prepare("SELECT detail FROM backup_log WHERE status='validated' AND detail IS NOT NULL").all()) {
    ...
    if (secondary?.ok && typeof secondary.filename === 'string') owned.add(secondary.filename);
  }
  return owned;
}
...
function cleanupBackupTempDirectory(directory, pattern, now) {
  ...
  for (const name of names) {
    ...
    if (!match || processMayBeRunning(Number(match[1]))) continue;
    const file = path.join(directory, name);
    try {
      const stat = fs.lstatSync(file);
      if (!stat.isFile() || stat.isSymbolicLink() || now - stat.mtimeMs < BACKUP_TEMP_MAX_AGE_MS) continue;
      fs.unlinkSync(file);
      removed++;
    } catch {}
  }
...
function cleanupAbandonedBackupTemps() {
  ...
  if (removed) writeAppLog('info', 'backups', `Removed ${removed} abandoned backup temporary file(s).`);
}
...
function validTimeZone(value) {
  ...
  catch { throw new Error('Notification timezone must be a valid IANA timezone, such as America/New_York.'); }
  return text;
}
...
function normalizeAppConfig(input, base = DEFAULT_APP_CONFIG) {
  ...
  if (!Number.isFinite(notificationCooldownMinutes) || notificationCooldownMinutes < 0 || notificationCooldownMinutes > 10080) throw new Error('Notification cooldown must be between 0 and 10080 minutes.');
  const emailDetailLevel = String(body.emailDetailLevel ?? base.emailDetailLevel).toLowerCase();
  if (!['compact', 'standard', 'detailed'].includes(emailDetailLevel)) throw new Error('Email detail level must be compact, standard, or detailed.');
  const emailTheme = String(body.emailTheme ?? base.emailTheme).toLowerCase();
  if (!['auto', 'light', 'dark'].includes(emailTheme)) throw new Error('Email theme must be auto, light, or dark.');
  const emailDigestMaxItems = Number(body.emailDigestMaxItems ?? base.emailDigestMaxItems);
  if (!Number.isInteger(emailDigestMaxItems) || emailDigestMaxItems < 1 || emailDigestMaxItems > 50) throw new Error('Email digest items must be between 1 and 50.');
  const emailSubjectPrefix = String(body.emailSubjectPrefix ?? base.emailSubjectPrefix).trim();
  if (/[\r\n]/.test(emailSubjectPrefix)) throw new Error('Email subject prefix must not contain line breaks.');
  if (emailSubjectPrefix.length > 60) throw new Error('Email subject prefix must be 60 characters or fewer.');
  const operationalAlerts = { ...base.operationalAlerts };
  for (const key of Object.keys(operationalAlerts)) if (typeof body.operationalAlerts?.[key] === 'boolean') operationalAlerts[key] = body.operationalAlerts[key];
...
function persistState(nextState, region = currentRegion(), { replaceEvents = false } = {}) {
  ...
  try {
    ...
    for (const row of db.prepare('SELECT slug FROM watchlist WHERE region=?').all(region)) {
      if (!wanted.has(row.slug)) db.prepare('DELETE FROM watchlist WHERE region=? AND slug=?').run(region, row.slug);
    }
...
function normalizeImageUrl(value) {
  ...
  try {
    ...
    if ((host === 'ui.com' || host.endsWith('.ui.com')) && /\.(png|jpe?g|webp|avif)(?:$|\/)/i.test(path)) return url.href;
  } catch {}
  return null;
}
...
function firstImage(product) {
  ...
  const score = (url) => {
    ...
    if (hostname === 'images.svc.ui.com') n += 50;
    if (hostname === 'cdn.ecomm.ui.com') n += 40;
    if (hostname === 'assets.ecomm.ui.com') n += 10;
...
function productDashboardUrl(slug, region = currentRegion()) {
  if (!PUBLIC_BASE_URL || !slug) return null;
  try {
...
function eventAlertKind(type, previous, current, rule) {
  if (type !== 'price_change') return type;
  ...
  if (crossedTarget) return 'target_price';
  if (prices.difference !== null && prices.difference < 0) return 'price_drop';
  return 'price_change';
}

function eventTriggerReason(type, alertKind, rule, watchedAtDetection) {
  if (alertKind === 'target_price') return `Price reached the configured target of ${rule.targetPrice}.`;
  if (alertKind === 'price_drop') return rule?.priceDropOnly ? 'Price-drop-only monitoring detected a lower price.' : 'The price decreased on a watched product.';
  if (type === 'price_change') return 'The price changed on a watched product.';
  if (type === 'restock') return rule?.immediateRestock ? 'Immediate restock alerts are enabled for this product.' : 'A watched product became available.';
  if (type === 'sold_out') return 'A watched product became unavailable.';
  if (type === 'status_change') return 'The store status changed on a watched product.';
  if (type === 'new_product') return 'New-product alerts are enabled for this store region.';
...
function originalNotificationCopy(event) {
  ...
  if (event.type === 'operational') return {
...
function notificationDecision(event, prefs = notificationPreferences(), ruleOverride = null) {
  ...
  if (suppressors.length) return { allowed:false, reason:'collection-only', rule, collections:suppressors };
  ...
  if (event.type === 'price_change') {
    ...
    if (rule.targetPrice !== null) enabled = crossedTarget;
    else if (rule.priceDropOnly) enabled = enabled && dropped;
  }
...
async function sendGotify(event) {
  if (!GOTIFY_BASE_URL || !GOTIFY_TOKEN) return null;
  const copy = notificationCopy(event);
...
function smtpAddress(value) {
  ...
  if (!address || /[\r\n<>]/.test(address)) throw new Error('Invalid SMTP address configuration.');
  return address;
}
...
function smtpReader(socket) {
  ...
  const onClose = () => { if (!failure) failure = new Error('SMTP connection closed unexpectedly.'); settle(); };
...
async function smtpResponse(reader, expectedCodes) {
  ...
  while (true) {
    ...
    if (!/^\d{3}-/.test(line)) throw new Error(`Invalid SMTP response: ${line}`);
  }
  const code = Number(lines[lines.length - 1].slice(0, 3));
  if (!expectedCodes.includes(code)) throw new Error(`SMTP ${code}: ${lines.join(' | ').slice(0, 500)}`);
  return lines;
}
...
async function connectSmtp() {
  ...
  if (SMTP_USER || SMTP_PASSWORD) {
    if (!(socket instanceof tls.TLSSocket) || !socket.encrypted) throw new Error('Refusing to send SMTP credentials without TLS.');
...
async function sendSmtp(event) {
  ...
  for (const warning of rendered.warnings) writeAppLog('warn', 'email', warning, { eventId:event.id });
  const { socket, reader, command } = await connectSmtp();
  try {
    await command(`MAIL FROM:<${smtpAddress(SMTP_FROM)}>`, [250]);
    for (const recipient of SMTP_TO) await command(`RCPT TO:<${smtpAddress(recipient)}>`, [250, 251]);
...
function previewNotificationEvent(slug, requestedType = 'restock') {
  if (!EMAIL_PREVIEW_TYPES.has(requestedType)) throw new Error('Unsupported email preview type.');
  if (requestedType === 'test') return testEvent();
  if (requestedType === 'operational') return {
  ...
  if (requestedType === 'digest') {
    ...
    if (events[0]) events.push({ ...events[0], id:'preview-duplicate' });
...
function quietAt(date) {
  if (!QUIET_HOURS_ENABLED || QUIET_HOURS_START === QUIET_HOURS_END) return false;
...
function deliveryPlan(event, rule = event.slug ? watchRule(event.slug, event.region || currentRegion()) : DEFAULT_WATCH_RULE) {
  const now = new Date();
  if (event.type === 'restock' && rule.immediateRestock) return { deliverAt:now.toISOString(), mode:'immediate-restock', timeZone:NOTIFICATION_TIME_ZONE };
...
function enqueueOperationalAlert(code, name, detail, region = currentRegion(), options = {}) {
  ...
  if (preference && !OPERATIONAL_ALERTS[preference]) return 0;
  ...
  try {
    ...
    for (const pendingRows of groups.values()) {
      ...
      try {
        ...
        if (terminalFailure) enqueueOperationalAlert('notifications', 'Notification delivery failed', `${rows[0].channel} exhausted its retry limit.`, rows[0].region, { excludeChannels:[rows[0].channel] });
      }
    }
...
function recordInventoryCoverage(at = isoNow()) {
  ...
  } catch (err) { db.exec('ROLLBACK TO inventory_coverage'); db.exec('RELEASE inventory_coverage'); throw err; }
}
...
function collectionReadiness(collection, region = currentRegion()) {
  ...
  const items = collection.slugs.map((slug) => {
    ...
    if (member.purchasedQuantity >= member.quantity) return { ...item, state:'purchased', reason:'Already purchased.' };
...
function evaluateCollectionAlerts() {
  for (const collection of watchCollections()) {
    ...
    } catch (err) { db.exec('ROLLBACK TO collection_readiness'); db.exec('RELEASE collection_readiness'); state.events = eventsBefore; throw err; }
  }
}
...
async function checkStore(reason = 'timer') {
  ...
  try {
    ...
    if (!monitor.partialErrors.length) {
      try { recoverFailedDeliveriesAfterOutage(currentRegion()); }
      catch (err) { writeAppLog('error', 'notifications', 'Delivery recovery after Store check failed.', { error:String(err?.message || err) }); }
    }
...
function scheduleMonitor() {
  const region = currentRegion();
  if (monitorTimers.has(region)) clearTimeout(monitorTimers.get(region));
...
function collectionMoney(value, label) {
  if (value === null) return null;
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1000000000) throw new Error(`${label} must be between 0 and 1,000,000,000, or empty.`);
  return Math.round(value * 100) / 100;
}

function normalizeCollectionItem(input, base = { quantity:1, purchasedQuantity:0, paidTotal:null }) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('Invalid collection item.');
  ...
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 1000) throw new Error('Quantity must be a whole number from 1 to 1000.');
  if (!Number.isInteger(purchasedQuantity) || purchasedQuantity < 0 || purchasedQuantity > quantity) throw new Error('Purchased quantity must be between zero and the desired quantity.');
  ...
  if (!purchasedQuantity && paidTotal !== null) throw new Error('Record a purchased quantity before entering the total paid.');
  ...
  return db.prepare('SELECT id,name,created_at AS createdAt,notify_ready AS notifyReady,ready_state AS readyState,budget,alerts_only AS alertsOnly,archived,budget_required AS budgetRequired,delivery_json AS deliveryJson FROM watch_collections WHERE region=? ORDER BY name COLLATE NOCASE').all(region)
    .map((collection) => {
      ...
      for (const item of items) {
        ...
        if (unknownPaid) missingPaid++;
        if (unknownPrice) missingPrices++;
        if (unknownPaid || unknownPrice) missing++;
        spentCents += Math.round((item.paidTotal || 0) * 100);
        if (unitPrice !== null) remainingCents += Math.round(unitPrice * 100) * remaining;
      }
...
function collectionMembers(value) {
  if (!Array.isArray(value) || value.length > 1000 || value.some((slug) => typeof slug !== 'string' || !state.watchlist.includes(slug))) throw new Error('Choose up to 1000 products from this region\'s watchlist.');
...
function saveCollectionMembers(id, slugs, additive = false) {
  ...
  if (existing.length === next.length && next.every((slug) => existing.includes(slug))) return;
  for (const slug of existing) if (!next.includes(slug)) db.prepare('DELETE FROM watch_collection_members WHERE region=? AND collection_id=? AND slug=?').run(region,id,slug);
  for (const slug of next) if (!existing.includes(slug)) insertCollectionMember(region,id,slug);
...
function collectWatchImportJson(value, output, parentKey = 'items', depth = 0) {
  ...
  if (typeof value === 'string' || typeof value === 'number') {
    if (depth <= 1 || WATCH_IMPORT_FIELDS.has(watchImportKey(parentKey)) || WATCH_IMPORT_CONTAINERS.has(watchImportKey(parentKey))) output.push(String(value));
    return;
  }
  if (Array.isArray(value)) {
  ...
  if (containers.length) {
    for (const [key, item] of containers) collectWatchImportJson(item, output, key, depth + 1);
    return;
  }
...
function parseWatchImportContent(content, fileName = '') {
  ...
  if (!text) watchImportError('Paste at least one Store link, product SKU, or slug.');
  if (Buffer.byteLength(text) > 200 * 1024) watchImportError('Watchlist imports must be 200 KB or smaller.');
  let references = [];
  const looksJson = /^\s*[\[{]/.test(text) || /\.json$/i.test(String(fileName));
  if (looksJson) {
    const parsed = safeJsonParse(text, null);
    if (parsed == null) watchImportError('The selected JSON file is not valid JSON.');
  ...
  if (!references.length) watchImportError('No product links, SKUs, or slugs were found in that import.');
  if (references.length > 1000) watchImportError('A watchlist import can contain at most 1,000 entries.');
...
function previewWatchImport(content, fileName = '') {
  ...
  const items = references.map((value, index) => {
    ...
    if (state.watchlist.includes(product.slug)) return { ...base, ...productData, status:'already', label:'Already watched', detail:'Already on this watchlist. Its existing alert rules will be preserved.' };
...
function scheduleBackups() {
  ...
  if (!BACKUP_INTERVAL_HOURS) return;
...
function securityWarnings() {
  const warnings = [];
  if (ACCESS_MODE === 'local' && !isLoopbackHost(BIND_HOST)) warnings.push({ severity:'high', code:'local-remote', settingsTab:'general', message:'Local mode is exposed beyond loopback without owner authentication.' });
  if (ACCESS_MODE !== 'local' && !ownerCredential()) warnings.push({ severity:'high', code:'owner-setup', settingsTab:'security', message:'Owner password setup is incomplete.' });
  if (ACCESS_MODE === 'private' && !COOKIE_SECURE && !PUBLIC_BASE_URL.startsWith('https://')) warnings.push({ severity:'medium', code:'plain-http', settingsTab:'general', message:'Remote access is using HTTP. Prefer a private VPN or authenticated HTTPS reverse proxy.' });
...
function exportSnapshot() {
  ...
  for (const region of ACTIVE_REGIONS) {
    const watchRules = {};
    for (const row of db.prepare('SELECT slug,rule_json FROM watch_rules WHERE region=?').all(region)) watchRules[row.slug] = normalizeWatchRule(safeJsonParse(row.rule_json, {}));
    const watchCreatedAt = {};
    for (const row of db.prepare('SELECT slug,created_at FROM watchlist WHERE region=?').all(region)) watchCreatedAt[row.slug] = row.created_at;
    regionData[region] = {
...
function encryptSnapshot(snapshot, passphrase) {
  if (typeof passphrase !== 'string' || passphrase.length < 12) throw new Error('Export passphrase must be at least 12 characters.');
...
function decryptSnapshot(wrapper, passphrase) {
  if (wrapper?.format !== 'GearBeaconEncryptedBackup') return wrapper;
  if (typeof passphrase !== 'string' || !passphrase) throw new Error('This backup is encrypted. Enter its passphrase.');
  try {
...
function normalizeImportedSnapshot(snapshot) {
  ...
  if (snapshot.regions && typeof snapshot.regions === 'object' && !Array.isArray(snapshot.regions)) {
    for (const [region, value] of Object.entries(snapshot.regions)) {
      if (REGIONS[region]) regions[region] = normalizeRegion(value);
    }
  } else {
...
function validateSnapshotCollections(regions) {
  ...
  for (const regionState of Object.values(regions)) {
    ...
    for (const collection of regionState.collections) {
      ...
      if (!Array.isArray(collection.items) || collection.items.length !== memberSlugs.length) throw new Error('Backup collection item plans do not match its members.');
      const itemSlugs = new Set();
      collection.items = collection.items.map((item) => {
        if (!item || !memberSlugs.includes(item.slug) || itemSlugs.has(item.slug)) throw new Error('Backup contains an unknown or duplicate collection item.');
...
function importSnapshot(snapshot) {
  ...
  for (const [region, regionState] of Object.entries(normalized.regions)) {
    ...
    for (const [slug, createdAt] of Object.entries(regionState.watchCreatedAt || {})) {
      ...
      if (regionState.watchlist.includes(slug) && !Number.isNaN(parsed.valueOf())) restoreWatchCreatedAt.run(parsed.toISOString(), region, slug);
    }
    ...
    for (const item of regionState.productHistory || []) {
      ...
      if (Number.isNaN(observed.valueOf())) continue;
...
function passwordHash(password, version = PASSWORD_HASH_VERSION) {
  ...
  if (!profile) throw new Error('Unsupported owner password hash version.');
  const salt = crypto.randomBytes(16);
...
function verifyPassword(password, stored) {
  try {
    ...
    const profile = PASSWORD_HASH_PROFILES[version];
    if (!profile || !saltText || !hashText) return false;
...
function upgradeOwnerPasswordHash(password, credential) {
  if (!credential || String(credential.password_hash || '').startsWith(`${PASSWORD_HASH_VERSION}$`)) return false;
  const now = isoNow();
...
function configuredOwnerPassword() {
  ...
  if (file) return fs.readFileSync(path.resolve(file), 'utf8').replace(/[\r\n]+$/, '');
...
function sessionForRequest(req) {
  if (!authenticationRequired()) return { local: true, csrf_token: null };
  cleanSessions();
...
function loginRateState(req) {
  ...
  if (loginAttempts.size > 5000) {
    for (const [address, value] of loginAttempts) {
      if (now - value.startedAt > 30 * 60 * 1000) loginAttempts.delete(address);
    }
    while (loginAttempts.size > 5000) loginAttempts.delete(loginAttempts.keys().next().value);
  }
  let item = loginAttempts.get(key);
  if (!item || now - item.startedAt > 15 * 60 * 1000) item = { count: 0, startedAt: now, blockedUntil: 0 };
...
function recordLoginFailure(req) {
  const { item, now } = loginRateState(req);
  item.count += 1;
  if (item.count >= 5) item.blockedUntil = now + 15 * 60 * 1000;
}
...
function validatedRequestHost(req) {
  ...
  if (requestFromLoopbackProxy(req)) {
    ...
    if (forwardedValue && !forwarded) return null;
    const effective = forwarded || direct;
    const configured = configuredProxyAuthorities();
    if (configured.has(effective.host) || isLoopbackHost(effective.hostname)) return effective.host;
    return null;
  }
...
function allowedRequestOrigin(req, requestHost) {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) return { allowed: false, origin };
    if (parsed.host.toLowerCase() === requestHost) return { allowed: true, origin };
    if (PUBLIC_BASE_URL && new URL(PUBLIC_BASE_URL).origin === parsed.origin) return { allowed: true, origin };
  } catch {}
...
function publicErrorMessage(err, status = 500) {
  if (status >= 500) return 'GearBeacon could not complete that request. Check Operations logs for details.';
...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/setup') {
    if (!setupRequired()) return sendJson(res, 409, { error: 'Owner setup has already been completed.' });
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/login') {
    if (!authenticationRequired()) return sendJson(res, 409, { error: 'Authentication is not enabled for this local-only instance.' });
    if (setupRequired()) return sendJson(res, 428, { error: 'Owner setup must be completed first.', setupRequired: true });
    try { assertLoginAllowed(req); } catch (err) { return sendJson(res, err.statusCode || 429, { error: err.message }); }
    const body = await readJsonBody(req);
  ...
  const session = sessionForRequest(req);
  if (!session) return sendJson(res, setupRequired() ? 428 : 401, { error: setupRequired() ? 'Owner setup is required.' : 'Authentication is required.', setupRequired: setupRequired() });
  ...
  if (req.method === 'DELETE' && url.pathname.startsWith('/api/auth/sessions/')) {
    ...
    if (!/^[a-f0-9]{16}$/.test(id)) return sendJson(res, 400, { error: 'Invalid session identifier.' });
    db.prepare('DELETE FROM sessions WHERE substr(token_hash,1,16)=?').run(id);
    const currentRevoked = session.token_hash?.startsWith(id);
    if (currentRevoked) res.gearbeaconHeaders = { ...commonResponseHeaders(res), 'Set-Cookie': expiredSessionCookie(req) };
  ...
  if (!ACTIVE_REGIONS.includes(requestedRegion)) return sendJson(res, 400, { error: `Region must be one of: ${ACTIVE_REGIONS.join(', ')}.` });
...
async function handleRegionApi(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/data/export/encrypted') {
    const body = await readJsonBody(req);
    let encrypted;
    try { encrypted = encryptSnapshot(exportSnapshot(), body?.passphrase); }
    catch (err) { return sendJson(res, 400, { error: err.message }); }
    const stamp = new Date().toISOString().slice(0, 10);
  ...
  if (req.method === 'GET' && url.pathname === '/api/notifications/preview') {
    ...
    let event;
    try { event = previewNotificationEvent(slug, eventType); }
    catch (err) { return sendJson(res, 400, { error:err.message }); }
  ...
  if (req.method === 'GET' && url.pathname === '/api/notifications/email-preview') {
    ...
    if (!['auto', 'light', 'dark'].includes(theme)) return sendJson(res, 400, { error:'Email preview theme must be auto, light, or dark.' });
    let event;
    try { event = previewNotificationEvent(slug, eventType); }
    catch (err) { return sendJson(res, 400, { error:err.message }); }
    let previewConfig;
  ...
  if (url.pathname === '/api/collections' && req.method === 'POST') {
    ...
    try { name = collectionName(body?.name); slugs = collectionMembers(body?.slugs ?? []); budget = collectionMoney(body?.budget === undefined ? null : body.budget, 'Budget'); } catch (err) { return sendJson(res, 400, { error:err.message }); }
  ...
  if (collectionItemMatch && ['POST','PUT','DELETE'].includes(req.method)) {
    ...
    if (req.method === 'DELETE') {
      ...
      try {
        ...
        baselineCollections(region,[id]); db.exec('COMMIT');
      } catch (err) { db.exec('ROLLBACK'); throw err; }
    } else {
      const body = await readJsonBody(req); let item, rule;
      try {
        item = normalizeCollectionItem(body, previous);
        if (body.targetPrice !== undefined) rule = normalizeWatchRule({ targetPrice:body.targetPrice, ...(body.targetPrice === null ? { availableUnderTarget:false } : {}) }, watchRule(slug));
      } catch (err) { return sendJson(res,400,{ error:err.message }); }
      db.exec('BEGIN IMMEDIATE');
      try {
        ...
        if (rule) saveWatchRule(slug,rule,region);
        if (item.quantity !== previous.quantity || item.purchasedQuantity !== previous.purchasedQuantity) baselineCollections(region,[id]);
        db.exec('COMMIT');
      } catch (err) { db.exec('ROLLBACK'); throw err; }
    }
  ...
  if (req.method === 'POST' && url.pathname === '/api/watch/import') {
    const body = await readJsonBody(req, 256 * 1024);
    if (!Array.isArray(body?.slugs)) return sendJson(res, 400, { error:'slugs must be an array.' });
    if (body.slugs.length > 1000) return sendJson(res, 400, { error:'A watchlist import can add at most 1,000 products.' });
    const slugs = [...new Set(body.slugs.map((value) => String(value || '').trim()).filter(Boolean))];
    if (!slugs.length) return sendJson(res, 400, { error:'Select at least one matched product to import.' });
    ...
    if (additions.length) {
      ...
      db.exec('BEGIN IMMEDIATE');
      try {
        for (const slug of additions) insert.run(currentRegion(), slug, createdAt);
        db.exec('COMMIT');
      } catch (err) {
        try { db.exec('ROLLBACK'); } catch {}
        throw err;
  ...
  if (req.method === 'POST' && url.pathname === '/api/watch') {
    ...
    if (!alreadyWatched || rule) {
      db.exec('BEGIN IMMEDIATE');
      try {
        if (!alreadyWatched) db.prepare('INSERT INTO watchlist(region,slug,created_at) VALUES(?,?,?) ON CONFLICT(region,slug) DO NOTHING').run(region, slug, isoNow());
        if (rule) saveWatchRule(slug, rule, region);
        db.exec('COMMIT');
      } catch (err) {
        try { db.exec('ROLLBACK'); } catch {}
  ...
  if (req.method === 'POST' && url.pathname === '/api/watch/bulk') {
    ...
    if (action === 'remove') {
      ...
      db.exec('BEGIN IMMEDIATE');
      try {
        for (const slug of slugs) saveWatchRule(slug, ruleChange);
        db.exec('COMMIT');
      } catch (err) {
        try { db.exec('ROLLBACK'); } catch {}
        throw err;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (!requestHost) return sendJson(res, 421, { error: 'Request host is not allowed.' });
    ...
    if (!origin.allowed) return sendJson(res, 403, { error: 'Request origin is not allowed.' });
    ...
    if (!res.headersSent) sendJson(res, status, { error:publicErrorMessage(err, status) });
    else res.end();
  }
});
...
async function start() {
  ...
  server.listen(PORT, BIND_HOST, () => {
    ...
    if (!isLoopbackHost(BIND_HOST)) console.log(`  Private URL: http://YOUR-SERVER-IP:${PORT} (use HTTPS through a reverse proxy outside a trusted LAN/VPN)`);
    console.log('');
  });
