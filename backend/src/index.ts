...
function alertConfiguration(product = null, collection = null) {
  ...
  if (state === 'active' && !channels.length) { state='no-channel'; reasons.unshift(label + '.'); label='No server notification channel enabled'; }
  if (!channels.length) reasons.push('Configure and enable a server channel in Settings > Notifications. Browser popups require an open page and browser permission.');
...
function normalizeImportedSnapshot(snapshot) {
  ...
  if (isBackup && Number(snapshot.formatVersion || 0) > 8) throw new Error(`This backup format (${snapshot.formatVersion}) is newer than GearBeacon ${APP_VERSION} supports.`);
...
async function handleRegionApi(req, res, url) {
  ...
  if (url.pathname === '/api/collections' && req.method === 'GET') return sendJson(res, 200, { ...watchWorkspace(), capabilities:{ memberEditing:true, purchasePlanning:true, watchWorkflow:true, budgetAlerts:true } });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/healthz') return sendJson(res, 200, { ok: true, name: 'GearBeacon', version: APP_VERSION, packageVersion: BUILD_INFO.packageVersion || APP_VERSION });
