...
function saveCollectionMembers(id, slugs, additive = false) {
  ...
  for (const slug of next) insert.run(region, id, slug);
...
async function handleRegionApi(req, res, url) {
  ...
  if (url.pathname === '/api/collections' && req.method === 'GET') return sendJson(res, 200, { collections:watchCollections(), capabilities:{ memberEditing:true } });
  if (url.pathname === '/api/collections' && req.method === 'POST') {
    const body = await readJsonBody(req);
    let name, slugs;
    try { name = collectionName(body?.name); slugs = collectionMembers(body?.slugs ?? []); } catch (err) { return sendJson(res, 400, { error:err.message }); }
