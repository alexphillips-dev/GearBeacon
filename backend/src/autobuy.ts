...
const text = (value, name, max = 160) => {
  if (typeof value !== 'string' || !value.trim() || value.length > max || /[\x00-\x1f\x7f]/.test(value)) fail(`Invalid ${name}.`);
  return value.trim();
};
...
function createAutoBuy({ db, regions, mock, getProduct, eligible, purchased, notify }) {
  ...
  function attention(row, code, state = 'attention') {
    ...
    if (current?.authorization === row.authorization) changeRule(current.id, state === 'unknown' ? 'attention' : 'paused', code);
  }
  function maintain() {
    const now = Date.now();
    for (const row of db.prepare("SELECT * FROM auto_buy_rules WHERE state='armed'").all()) {
      const config = JSON.parse(row.config_json);
      if (Date.parse(config.expiresAt) <= now) pause(row.id, 'expired');
      else if (!eligible(row.region, row.slug, config.collectionId, config.quantity)) pause(row.id, 'removed');
    }
    const busy = active();
  ...
  function pause(id, code = 'paused') {
    const row = ruleRow(id); if (!row) fail('Auto-buy rule not found.', 404);
    tx(() => {
      if (row.state !== 'purchased') changeRule(id, code === 'expired' ? 'expired' : 'paused', code);
  ...
  function pauseAll(code = 'paused') {
    for (const row of db.prepare("SELECT id FROM auto_buy_rules WHERE state IN ('armed','attention')").all()) pause(row.id, code);
    return { ok:true, submitting:Boolean(active()) };
  }
  function profileFor(region) {
    const c = connection();
    if (!c || Date.now() - Date.parse(c.last_seen || '') > 30000) fail(reasons.connection, 409);
    const p = JSON.parse(c.profiles_json)[region];
    if (!p || p.state !== 'ready') fail(reasons.session, 409);
    return p;
  }
  function save(region, slug, body) {
    maintain();
    if (body.authorized !== true) fail('Explicit purchase authorization is required.');
    const product = getProduct(region, slug);
    if (!product?.variantId || !product.sku || product.unlisted) fail('Choose an exact, listed product variant for auto-buy.');
    const url = new URL(product.url);
    if (url.protocol !== 'https:' || url.origin !== regions[region].origin || url.username || url.password || url.hash) fail('Invalid Store product URL.');
    const previous = db.prepare('SELECT * FROM auto_buy_rules WHERE region=? AND slug=?').get(region, slug);
    if (!previous && db.prepare('SELECT COUNT(*) AS count FROM auto_buy_rules').get().count >= 1000) fail('At most 1000 purchase instructions are supported.');
    if ((previous?.revision || 0) !== body.revision) fail('This purchase instruction changed. Reopen it before saving.', 409);
    if (active()) fail('Finish or resolve the current checkout before arming another instruction.', 409);
    const profile = profileFor(region);
    if (typeof body.expiresAt !== 'string' || !Number.isFinite(Date.parse(body.expiresAt))) fail('Choose a valid expiry.');
    const expiresAt = new Date(body.expiresAt).toISOString();
    if (Date.parse(expiresAt) <= Date.now() || Date.parse(expiresAt) > Date.now() + 90 * 86400000) fail('Choose an expiry within the next 90 days.');
    const config = {
    ...
    if (!eligible(region, slug, config.collectionId, config.quantity)) fail('Watch this exact variant and select a purchase plan with enough remaining quantity.');
  ...
  function observe(region, products) {
    ...
    for (const row of db.prepare("SELECT * FROM auto_buy_rules WHERE region=? AND state='armed'").all(region)) {
      ...
      if (!product?.inStock || product.unlisted || product.variantId !== config.variantId || product.sku !== config.sku) continue;
  ...
  function authenticate(token) {
    const c = connection();
    if (!c || typeof token !== 'string' || token.length > 200 || hash(token) !== c.token_hash) fail('Checkout companion authentication failed.', 401);
    return c;
  }
  function heartbeat(c, body) {
    if (body.protocol !== 1 || body.mode !== c.mode) fail('Checkout companion protocol or mode does not match.', 409);
    const profiles = {};
    for (const [region, p] of Object.entries(body.profiles || {})) {
      if (!Object.hasOwn(regions,region) || !p || !['ready','attention'].includes(p.state)) fail('Invalid checkout profile.');
    ...
    if (body.attemptId) db.prepare("UPDATE auto_buy_attempts SET updated_at=? WHERE id=? AND generation=? AND state IN ('preparing','submitting')").run(stamp(), body.attemptId, c.generation);
    maintain();
    return { ok:true };
  }
  function claim(c) {
    ...
    return tx(() => {
      ...
      for (const row of db.prepare("SELECT * FROM auto_buy_attempts WHERE state='queued' ORDER BY created_at,id").all()) {
        ...
        if (r?.state !== 'armed' || r.authorization !== row.authorization) { attention(row,'paused','cancelled'); continue; }
        let profile; try { profile = profileFor(row.region); } catch { continue; }
        if (profile.id !== config.profileId) { attention(row,'session'); continue; }
        // Stock is rechecked from a fresh complete catalog, and again in checkout.
        const product = getProduct(row.region, row.slug);
        if (!product?.inStock || product.unlisted || !product.autoBuyFresh) continue;
  ...
  function owned(c, id) {
    ...
    if (!row || row.generation !== c.generation) fail('Checkout attempt not found for this companion.', 404);
    return row;
  }
  function validateProof(row, proof) {
    ...
    if (!proof || proof.profileId !== config.profileId || proof.currency !== config.currency || proof.taxCalculated !== true || proof.shippingSelected !== true
      ...
      || typeof proof.observedAt !== 'string' || Math.abs(Date.now() - Date.parse(proof.observedAt)) > 10000 || !Number.isFinite(Date.parse(proof.observedAt))) fail(reasons.cart, 409);
  ...
  function authorize(c, id, proof) {
    ...
    return tx(() => {
      ...
      if (row.state !== 'preparing' || r?.state !== 'armed' || r.authorization !== row.authorization || Date.parse(config.expiresAt) <= Date.now()
        || !eligible(row.region,row.slug,config.collectionId,config.quantity) || profileFor(row.region).id !== config.profileId) fail('Purchase authorization is no longer active.',409);
      const product = getProduct(row.region,row.slug);
      if (!product?.autoBuyFresh || !product.inStock || product.sku !== config.sku || product.variantId !== config.variantId) fail('Waiting for a fresh confirmation of this exact variant.',409);
  ...
  function complete(c, id, receipt) {
    ...
    if (row.state === 'purchased') return { ok:true, attempt:publicAttempt(row) };
    if (!['submitting','unknown'].includes(row.state)) fail('This attempt was not authorized to submit.',409);
    ...
    if (!proof || receipt?.paid !== true || receipt.checkoutId !== proof.checkoutId || receipt.currency !== proof.currency || receipt.totalMinor !== proof.totalMinor
      || JSON.stringify(receipt.items) !== JSON.stringify(proof.items)) fail('The Store has not confirmed the authorized order and payment.',409);
  ...
  function report(c, id, body) {
    const row = owned(c,id);
    if (!['preparing','submitting'].includes(row.state)) return { ok:true, state:row.state };
  ...
  function resolve(id, body) {
    const row = attemptRow(id);
    if (!row || row.state !== 'unknown') fail('Only an uncertain submission needs owner reconciliation.',409);
    if (body.checkedStoreOrders !== true) fail('Check Store orders and payment activity before resolving this attempt.');
    // A running companion could still be receiving the payment result. Revoke it first.
    if (connection()) fail('Disconnect the companion and close its checkout browser before resolving an uncertain order.',409);
    ...
    } else fail('Choose whether an order was placed.');
    return { ok:true };
  }
  function disconnect() {
    pauseAll();
    const row = active();
    if (row?.state === 'submitting') tx(() => attention(row,'unknown','unknown'));
    db.prepare('DELETE FROM auto_buy_connection').run();
    pairing = null;
    return { ok:true };
  }
  function startPairing() {
    if (connection() || active()) fail('Disconnect the existing companion and resolve any pending order first.',409);
  ...
  function pair(body) {
    if (!pairing || Date.now() > pairing.expires || typeof body.code !== 'string' || hash(body.code) !== pairing.hash) fail('Pairing code is invalid or expired.',401);
    if (body.mode !== (mock ? 'mock' : 'live') || body.protocol !== 1) fail('Companion mode or version does not match GearBeacon.',409);
  ...
  function validateImport(data) {
    ...
    if (!data || !Array.isArray(data.rules) || !Array.isArray(data.attempts) || data.rules.length > 1000 || data.attempts.length > 100000) fail('Invalid auto-buy backup.');
    for (const row of [...data.rules,...data.attempts]) {
      if (!row || !Object.hasOwn(regions,row.region) || !/^[a-f0-9-]{36}$/.test(row.id) || !['armed','paused','attention','purchased','expired','queued','preparing','submitting','unknown','cancelled'].includes(row.state)) fail('Invalid auto-buy backup record.');
      for (const key of ['slug','sku','variantId','name','url','profileId','addressLabel','paymentLabel']) text(row[key],`backup ${key}`,2048);
      if (!Number.isFinite(Date.parse(row.expiresAt)) || !Number.isFinite(Date.parse(row.updatedAt)) || row.currency !== regions[row.region].currency || !['mock','live'].includes(row.mode)) fail('Invalid auto-buy backup values.');
      positive(row.quantity,'backup quantity',20); positive(row.maxTotalMinor,'backup limit',100000000);
      if (row.collectionId !== null) text(row.collectionId,'backup collection');
      if (row.ruleId !== undefined && !/^[a-f0-9-]{36}$/.test(row.ruleId)) fail('Invalid backup purchase rule identifier.');
      if (row.receipt) { text(row.receipt.orderNumber,'backup order number'); positive(row.receipt.totalMinor,'backup order total',100000000); }
    }
  }
  function importData(data) {
    validateImport(data); restore();
    if (!data) return;
    ...
    tx(() => {
      for (const row of data.rules) {
        if (!getProduct(row.region,row.slug)) continue;
...
function neutralizeAutoBuyBackup(db) {
  if (!db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='auto_buy_rules'").get()) return;
