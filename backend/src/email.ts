...
function cleanHeader(value, fallback = '') {
  ...
  if (/[\r\n]/.test(text)) throw new Error('Email headers must not contain line breaks.');
...
function emailKind(event) {
  if (event.alertKind) return event.alertKind;
  if (event.type === 'price_change') {
    const prices = priceChange(event);
    if (event.targetPrice !== null && event.targetPrice !== undefined && prices.current !== null && prices.current <= Number(event.targetPrice)) return 'target_price';
...
function reasonText(event) {
  if (event.triggerReason) return String(event.triggerReason);
  const kind = emailKind(event);
  if (kind === 'target_price') return `The price met your configured target${event.targetPrice != null ? ` of ${event.targetPrice}` : ''}.`;
  if (kind === 'price_drop') return 'The price decreased on a product you monitor.';
  if (kind === 'restock') return event.watchedAtDetection ? 'A product you monitor became available.' : 'GearBeacon detected that this product became available.';
  if (kind === 'sold_out') return 'A product you monitor became unavailable.';
  if (kind === 'new_product') return 'New-product alerts are enabled for this store region.';
  if (kind === 'operational') return 'Operational alerts are enabled for this GearBeacon installation.';
  if (kind === 'test') return 'You requested a test from GearBeacon notification settings.';
  return 'A monitored product changed on the UniFi Store.';
}

function moneyDelta(event) {
  const values = priceChange(event);
  if (values.difference === null) return null;
  ...
  if (values.difference < 0) return `Save ${absolute}${values.percent !== null ? ` (${Math.abs(values.percent)}%)` : ''}`;
  if (values.difference > 0) return `Up ${absolute}${values.percent !== null ? ` (${Math.abs(values.percent)}%)` : ''}`;
...
function imageSource(event, options) {
  ...
  if (options.imageSources?.[source]) return options.imageSources[source];
...
function productImage(event, options, size = 184) {
  ...
  if (source) return `<img src="${htmlEscape(source)}" width="${size}" alt="${htmlEscape(event.name || 'Product')}" style="display:block;width:100%;max-width:${size}px;height:auto;margin:0 auto;border:0;object-fit:contain">`;
...
function detailRows(event, options) {
  if (options.detailLevel === 'compact') return '';
  const rows = [
  ...
  if (event.targetPrice !== null && event.targetPrice !== undefined) rows.push(['Target price', String(event.targetPrice)]);
  if (event.previousStatus) rows.push(['Previous status', event.previousStatus]);
...
function textForEvent(event, options) {
  ...
  if (event.previousStatus || event.status) lines.push(`${event.previousStatus || 'Status'} → ${event.status || 'Updated'}`);
  if (event.price) lines.push(`Price: ${event.price}${event.previousPrice ? ` (was ${event.previousPrice})` : ''}`);
  if (options.priceCalculations && moneyDelta(event)) lines.push(moneyDelta(event));
  if (event.type !== 'collection_ready' && (event.sku || event.slug)) lines.push(`SKU: ${event.sku || event.slug}`);
  ...
  if (event.dashboardUrl) lines.push(`Open in GearBeacon: ${event.dashboardUrl}`);
...
async function fetchInlineImage(value, remainingBytes = MAX_INLINE_TOTAL_BYTES) {
  ...
  if (!url || remainingBytes <= 0) return null;
  ...
  try {
    ...
    if (!INLINE_IMAGE_TYPES.has(type)) throw new Error(`unsupported content type ${type || 'unknown'}`);
    const contentLength = Number(response.headers.get('content-length') || 0);
    const limit = Math.min(MAX_INLINE_IMAGE_BYTES, remainingBytes);
    if (contentLength && contentLength > limit) throw new Error('image is too large');
    ...
    while (true) {
      ...
      if (done) break;
      size += chunk.length;
      if (size > limit) { try { await reader.cancel(); } catch {} throw new Error('image is too large'); }
...
async function buildMimeEmail(event, input) {
  ...
  if (input.iconPath) {
    ...
    } catch (error) { warnings.push(`Logo was not embedded: ${error?.message || error}`); }
  }
  if (input.embedImages !== false) {
    ...
    for (const url of urls) {
      ...
      } catch (error) { warnings.push(`Product image was not embedded: ${error?.message || error}`); }
    }
  }
  ...
  if (Buffer.byteLength(message) > 9 * 1024 * 1024) throw new Error('Rendered email exceeds the 9 MB safety limit.');
