...
function windowsOperation(file, operation, value = '') {
  try {
    return execFileSync(path.join(process.env.SystemRoot || 'C:\\Windows', 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe'),
  ...
  } catch { throw new Error('Unable to secure local data for this Windows account. Check data-directory permissions and run under the original service or user account.'); }
}
function secureDataDirectory(directory) {
  const stat = fs.lstatSync(directory);
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('The data directory must be a real directory.');
  if (process.platform === 'win32') windowsOperation(directory, 'acl');
  else {
    fs.chmodSync(directory, 0o700);
    if ((fs.statSync(directory).mode & 0o777) !== 0o700) throw new Error('Unable to restrict the data directory to its owner.');
  }
}
let cachedWindowsKey;
function loadProtectedKey(file) {
  if (process.platform === 'win32' && cachedWindowsKey?.file === file) return cachedWindowsKey.key;
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error('Secret key path is not a regular file.');
  if (process.platform !== 'win32') {
    fs.chmodSync(file, 0o600);
    if ((fs.lstatSync(file).mode & 0o777) !== 0o600) throw new Error('Unable to restrict the secret key to its owner.');
  } else windowsOperation(file, 'acl');
  const stored = fs.readFileSync(file, 'utf8').trim();
  const protectedKey = stored.startsWith('dpapi-v1:');
  if (protectedKey && process.platform !== 'win32') throw new Error('This key belongs to its original Windows account. Use an encrypted transfer export to move installations.');
  ...
  if (process.platform === 'win32') {
    if (!protectedKey) {
      ...
      try {
        ...
        if (!key.equals(Buffer.from(windowsOperation(temporary, 'unprotect', wrapped), 'base64'))) throw new Error('Key protection verification failed.');
