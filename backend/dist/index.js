...
function createDatabaseBackup(reason = 'manual') {
    ...
    try {
        ...
        if (tableExists('backup_log'))
            db.prepare('INSERT INTO backup_log(filename,reason,status,size,detail,created_at) VALUES(?,?,?,?,?,?)').run(filename, reason, 'validated', size, null, isoNow());
    ...
    backupTimer = setTimeout(() => {
        try {
            ...
            if (backup) {
                ...
                writeAppLog('info', 'backups', 'Scheduled backup created and validated.', { filename: backup.filename, size: backup.size });
            }
        }
        catch (err) {
