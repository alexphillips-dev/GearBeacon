...
function scrubSupportValue(value, key = '') {
    if (/password|secret|token|credential|session|cookie|authorization|path$|directory$|dir$/i.test(key))
        return '[redacted]';
    if (/^(?:name|slug|slugs|sku|variantId|variantSlug|variantTitle|variantKeys|parentSlug|sourceSlug|watchlist|watchRules|collections|collectionName|candidate|product|products|productHistory)$/i.test(key))
        return '[redacted]';
    if (Array.isArray(value))
...
function normalizeImportedSnapshot(snapshot) {
    ...
    if (isBackup && Number(snapshot.formatVersion || 0) > 4)
