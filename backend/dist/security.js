...
function createOwnerSecurity({ db, getSetting, setSetting, encrypt, decrypt, sessionHours, idleMinutes }) {
    ...
    function validatePolicy(input) {
        ...
        if (!Number.isInteger(value.sessionHours) || value.sessionHours < 1 || value.sessionHours > 168 || !Number.isInteger(value.idleMinutes) || value.idleMinutes < 1 || value.idleMinutes > 120)
            throw new Error('Session lifetime must be 1–168 hours; idle lock must be 1–120 minutes.');
        return value;
    }
        ...
        if (/^\d{6}$/.test(text)) {
            for (const offset of [-1, 0, 1]) {
                ...
                if (candidate > (value.lastStep ?? -1) && crypto.timingSafeEqual(Buffer.from(text), Buffer.from(totp(value.secret, candidate)))) {
                    saveMfa({ ...value, lastStep: candidate });
                    return true;
                }
            }
        }
    ...
    function finish(session, code) {
        ...
        if (!pending.secret || pending.session !== session.token_hash || pending.expires < Date.now())
        ...
        if (matched === undefined)
            throw new Error('The authenticator code is incorrect.');
    ...
    function savePolicy(input) {
        ...
        setSetting('owner_security_policy', JSON.stringify(value));
        return value;
    }
