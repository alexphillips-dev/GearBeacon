...
- **New listings and availability:** teal **NEW TO STORE** badges identify new-product discovery events in the monitored regional catalog. The description includes the listing's availability at detection, such as Coming soon or In stock. **NOW AVAILABLE** marks a Coming soon → In stock event and uses Coming soon for the observed waiting duration. Regular restocks keep Back after and have no discovery badge. These badges describe the recorded event, so they remain when the separate current status changes; they do not establish a manufacturer's release date.
