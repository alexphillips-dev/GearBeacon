...
- Activity keeps its current page while new events arrive and offers **New activity available** to load the latest entries. Pagination retains a server snapshot boundary; delivery/filter or retention changes offer a refresh without replacing the page being read. Existing 20/50/100 pagination and 64-pixel rows remain supported.
...
- Added a Browse/product destination picker with exact variants, Watchlist or collection destinations, inline collection creation, and desired quantities. Add saves watch and membership together, reuses existing watches, and keeps an existing membership's quantities, payments, and alert rules.
- Added archiving and restoration through collection editing and Manage collections, with Active/Archived/All filters. Archived projects retain purchase records and saved settings, stop collection deliveries, release collection-only overrides, and stay out of active readiness summaries. Restoring establishes a fresh alert baseline; global watch purchase actions preserve archived spending.
- Added persistent-in-page Undo after collection item removal, including quantity and actual spending recovery. Undo never overwrites a newer membership and remains available after closing/reopening the manager until dismissed or the page is refreshed.
- Added transactional schema-v11 migration with a validated safety backup and format-v7 recovery exports preserving archive state. Existing collections start active; previous purchase plans and older recovery formats remain supported.
- Added collection purchase plans with desired quantities, partial purchases, recorded actual spending, optional budgets, remaining-cost estimates, over/under-budget status, and purchased-unit progress. Recorded payments remain fixed when catalog prices change; unknown payments or prices produce an explicitly incomplete subtotal.
- Added Edit item, Record purchase, Store, and Remove actions to collection detail rows. Quantity and spending records apply to the project; target-price edits apply to the shared watch. Membership saves preserve retained items' purchase records, and removing a member keeps its watch and other memberships.
- Added opt-in Collection alerts only mode. Enabled collections suppress their members' individual notifications, including All activity and immediate restocks, without modifying saved rules. Overlapping overrides are named in the UI; removing the last active override restores future item alerts. Queued or failed individual alerts are cancelled, and delivery checks suppression again before sending.
- Added backup-protected schema-v10 migration and format-v6 recovery for purchase plans, budgets, and alert modes. Existing members start at quantity one and preserve purchased state with unknown actual costs; older backups remain supported. The updated page verifies server purchase-planning support before saving.
...
- Replaced wide collection readiness panels with product-sized cards in the Watchlist grid. Collections preview up to four assigned products, show an additional-item count and combined catalog price, and keep View items, Alerts, and Edit actions on the card. Open the image or name for collection details. Totals combine recorded spending and remaining-unit catalog costs, use the collection's store currency, and disclose unknown costs instead of presenting a partial subtotal as complete.
- Fixed false collection-save success when an updated page connects to an older GearBeacon process. The editor now checks that the server supports membership edits and verifies the returned items before confirming success; failed saves retain selections and explain when a server restart is needed.
- Redesigned Manage collections as a focused dialog beside Import watchlist, with a guided empty state, collection counts, and View watches/Edit actions. Create and edit collections with a searchable watch picker, thumbnails, and variant details; add selected watches to existing or new collections from the bulk toolbar. Saves update the name and membership together, cancellation keeps existing data, and deletion explains that watches, rules, and history remain.
