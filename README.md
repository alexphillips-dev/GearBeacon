...
Live data stays in the operating system's application-data directory or Docker `/data`, outside application files. SQLite backups are listed only after validation, and a failed portable restore leaves the existing database state intact. SQLite backups and the separate `secrets.key` preserve saved state and encrypted integration credentials. Windows protects this key for the original account using DPAPI; use encrypted exports for transfers to another machine or account. See [owner security and recovery](docs/SECURITY_CONTROLS.md) for authenticator setup, session locking, backups, and verified updates. Portable encrypted exports exclude owner credentials, sessions, and local integration secrets. Auto-buy instructions restore paused. Checkout sessions and their encryption key stay on the companion host and are excluded from GearBeacon backups.
